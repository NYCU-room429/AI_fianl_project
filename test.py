...
def test(
    model_path,
    test_data_root, # Changed to root path for data collection
    class_json, # Path to the instrument class JSON
    batch_size=1,
    device="cuda",
    num_tracks=None, # Process all tracks by default unless specified
):
    # Collect test file paths
    # is_test=True ensures only mix.flac and all_src.mid are considered per track folder
    ...
    instrument_classes_list = get_all_class(class_json) # e.g. ["Piano", "Guitar", "Drums"]
    ...
    test_loader = torch.utils.data.DataLoader(
        test_dataset, batch_size=batch_size, shuffle=False, collate_fn=collate_fn_skip_nones
    )
    ...
    if os.path.exists(thresholds_path):
        try:
            ...
            if len(loaded_thresholds) == num_classes:
                ...
                print(f"Warning: Mismatch in number of classes. {thresholds_path} has {len(loaded_thresholds)} thresholds, model has {num_classes}. Using default 0.5.")
        except Exception as e:
            print(f"Warning: Could not load or parse {thresholds_path}: {e}. Using default 0.5.")
    else:
        print(f"Warning: {thresholds_path} not found. Using default threshold of 0.5 for all classes.")
    ...
    with torch.no_grad():
        for batch_idx, (mel, label) in enumerate(tqdm(test_loader, desc="Testing")):
            if mel is None or label is None: # Skipped by collate_fn, but double check
            ...
            ).unsqueeze(0).unsqueeze(0) # Reshape for broadcasting: (1, 1, num_classes)
            
            preds_proba = torch.sigmoid(outputs)
            # preds is now a boolean array indicating active frames per class
            preds_active_frames = (preds_proba > threshold_tensor).cpu().numpy() # (batch, time_frames, num_classes)
            label_np = label.numpy()  # (batch, time_frames, num_classes)

            for i in range(preds_active_frames.shape[0]): # Iterate through tracks in the batch
                current_preds_active_frames = preds_active_frames[i] # (time_frames, num_classes)
                ...
                label_inst_present_indices = np.where(current_label_active_frames.any(axis=0))[0]
                
                pred_inst_present_names = [instrument_classes_list[k] for k in pred_inst_present_indices]
                label_inst_present_names = [instrument_classes_list[k] for k in label_inst_present_indices]
                ...
                if processed_tracks < len(test_midi_file_list):
                    # Attempt to get a more descriptive track name from the path
                    try:
                        track_folder_name = test_midi_file_list[processed_tracks].split(os.sep)[-3] # e.g. Track00001
                        stem_name = os.path.splitext(os.path.basename(test_midi_file_list[processed_tracks]))[0]
                        if stem_name == "all_src":
                            track_identifier = f"Track: {track_folder_name} (Mix)"
                        else:
                            track_identifier = f"Track: {track_folder_name} (Stem: {stem_name})"
                    except IndexError:
                ...
                print(f"  Ground Truth Instrument Classes (Present): {sorted(list(set(label_inst_present_names)))}")
                print(f"  Predicted Instrument Classes (Present):  {sorted(list(set(pred_inst_present_names)))}")
                print(f"  Predicted Longest Duration Instrument(s): {sorted(longest_duration_pred_names)} (active for {max_pred_frames} frames)")


                # Frame-wise metrics for this track
                track_f1_micro = f1_score(current_label_active_frames, current_preds_active_frames, average="micro", zero_division=0)
                track_precision_micro = precision_score(current_label_active_frames, current_preds_active_frames, average="micro", zero_division=0)
                track_recall_micro = recall_score(current_label_active_frames, current_preds_active_frames, average="micro", zero_division=0)
                
                print(f"  Track Micro F1: {track_f1_micro:.4f} | Precision: {track_precision_micro:.4f} | Recall: {track_recall_micro:.4f}")
...
if __name__ == "__main__":
    test(
        ...
        test_data_root=os.path.join("slakh2100_flac_redux", "slakh2100_flac_redux", "test"),
        class_json=CLASS_JSON_PATH, 
        batch_size=1,
        device="cuda" if torch.cuda.is_available() else "cpu",
        num_tracks=None # Set to a small number for quick test, or None for all
    )