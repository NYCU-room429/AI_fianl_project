...
def main():
    ...
    train_midi_file_list, train_flac_file_list = collect_all_path(
        "D:\\AI_fianl_project\\slakh2100_flac_redux\\slakh2100_flac_redux\\train"
    )
    val_midi_file_list, val_flac_file_list = collect_all_path(
        "D:\\AI_fianl_project\\slakh2100_flac_redux\\slakh2100_flac_redux\\validation"
    )
    ...
    all_labels = np.concatenate(
        all_labels, axis=0
    )  # (total_samples, time, num_classes)
    ...
    for epoch in range(EPOCHS):  # epoch
        ...
        for class_idx in tqdm.tqdm(range(num_classes), desc="Optimizing thresholds"):
            ...
            for thresh in np.arange(0.01, 1.0, 0.01):
                class_preds = (
                    torch.sigmoid(torch.tensor(class_logits)) > thresh
                ).numpy()
