...
def _get_or_load_instrument_data(json_path):
    ...
    if json_path not in _instrument_data_cache:
        if not os.path.exists(json_path):
            raise FileNotFoundError(f"Instrument JSON definition file not found: {json_path}")
        try:
            with open(json_path, "r", encoding='utf-8') as f:
                data = json.load(f)
        except json.JSONDecodeError as e:
            raise ValueError(f"Error decoding JSON from {json_path}. Ensure it's a single valid JSON object. Details: {e}")
        ...
        for prog_key_str, details in data.items():
            ...
            if prog_key_str == "128": # Assuming "128" is the conventional key for the "Drums" class entry
...
def gen_melgram(path, num_frame=1000, segment_sec=100, is_train=False):
    ...
    end_sec = min(total_sec, start_sec + segment_sec) # Ensure segment is segment_sec long if possible
    ...
    if (end_sec - start_sec) < 10.0: # Example: require at least 10s for a meaningful segment
    ...
    hop_length = (len(src_segment) - N_FFT) // (num_frame -1) if num_frame > 1 else N_FFT # Avoid div by zero
    ...
    mel_db = librosa.power_to_db(mel, ref=np.max) # Use ref=np.max for better normalization before instance norm

    # Pad or truncate time dimension (mel_db.shape[1]) to num_frame
    if mel_db.shape[1] < num_frame:
        pad_width = num_frame - mel_db.shape[1]
        mel_db = np.pad(mel_db, ((0, 0), (0, pad_width)), mode="constant", constant_values=np.min(mel_db)) # Pad with min value
    ...
    if is_train: # SpecAugment
        ...
        for _ in range(num_freq_mask):
            if N_MELS > freq_mask_max_width: # Ensure N_MELS is large enough for mask
                ...
                f_end = f_start + np.random.randint(1, freq_mask_max_width) # Mask at least 1 bin
                mel_db[f_start:f_end, :] = np.min(mel_db) # Mask with min value of spectrogram
        ...
        for _ in range(num_time_mask):
            if num_frame > time_mask_max_width: # Ensure num_frame is large enough
                ...
                t_end = t_start + np.random.randint(1, time_mask_max_width) # Mask at least 1 frame
                mel_db[:, t_start:t_end] = np.min(mel_db) # Mask with min value

    return mel_db


def extract_label_from_midi(midi_path, target_classes, program_class_mapping, num_frame=1000, segment_sec=100):
    try:
    ...
    for instr in midi.instruments:
        ...
        if instr.is_drum:
            ...
            instrument_category = program_class_mapping.get("Drums_specific_class", "Drums")
        else:
        ...
        for note in instr.notes:
            ...
            if eff_note_end_in_segment <= eff_note_start_in_segment or \
               eff_note_end_in_segment <= segment_start_time or \
               eff_note_start_in_segment >= segment_end_time:
                continue
            ...
            for i in range(start_frame_idx, end_frame_idx + 1): # Inclusive of end_frame_idx
